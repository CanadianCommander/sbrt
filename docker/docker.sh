...
if [[ ${OPERATION} == "dev" ]] || [[ ${OPERATION} == "generate_api" ]]; then
  if [[ ${REBUILD} == true ]]; then
    ...
    sudo -E DOCKER_BUILDKIT=1 docker build --no-cache -f {{project-name}}_${OPERATION}/spring_boot.Dockerfile ../ -t "{{project-name}}_${OPERATION}_spring_boot" --build-arg USER_ID=${USER_ID} --build-arg USER_GROUP_ID=${USER_GROUP_ID}
  fi
  sudo -E docker-compose -f {{project-name}}_${OPERATION}/docker-compose.yml up --abort-on-container-exit
elif [[ ${OPERATION} == "deploy_client_libraries" ]]; then
    if [[ ${REBUILD} == true ]]; then
      sudo -E docker container rm {{project-name}}_${OPERATION}_deploy_clients_1
      sudo -E DOCKER_BUILDKIT=1 docker build --no-cache -f {{project-name}}_${OPERATION}/deploy_clients.Dockerfile ../ -t "{{project-name}}_${OPERATION}_deploy_clients" --build-arg USER_ID=${USER_ID} --build-arg USER_GROUP_ID=${USER_GROUP_ID}
    fi
    sudo -E docker-compose -f {{project-name}}_${OPERATION}/docker-compose.yml up --abort-on-container-exit
  ...
  sudo -E docker tag {{project-name}}_prod_spring_boot $IMAGE_PATH
